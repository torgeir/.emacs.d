...
values."
  (setq-default
   ...
   dotspacemacs-additional-packages '(powerline all-the-icons)
...
`dotspacemacs/user-config' first."
  ...
  (add-to-list 'configuration-layer--elpa-archives '("org" . "http://orgmode.org/elpa/"))
  (add-to-list 'configuration-layer--elpa-archives '("melpa-stable" . "stable.melpa.org/packages/"))

  (load "~/.emacs.d/t-bootstrap.el")
  (load "~/.emacs.d/setup/t-modules.el"))
...
you should place your code here."
  ...
  (dolist (pkg t-use-package-pkgs)
    (eval `(with-eval-after-load (quote ,pkg)
             (t/call-init "t/config-%s" (symbol-name (quote ,pkg))))))

  (dolist (module t-modules)
    (t/call-init "%s/config" module))

  ;; TODO not ideal, fix :bind in t/use-package instead for packages
  ;; that spacemacs already load, but this fixes the warnings on spacemacs
  ;; load, about packages that double in spacemacs and the t "layer"
  (dolist (pkg (-difference t-use-package-pkgs t-use-package-t-layer-pkgs))
    (eval `(with-eval-after-load (quote ,pkg)
             (t/call-init "t/init-%s" (symbol-name (quote ,pkg)))))))
