...
- Start by trying to evaluate some elisp once per session. When interacting with Emacs use `emacsclient`, use the `t-emacs` server
  - emacsclient -s t-emacs -e '(message "hello from ai")'
